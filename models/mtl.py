...
class MtlLearner(nn.Module):
    ...
    def __init__(self, args, mode='meta', num_cls=64):
        ...
        z_dim = 2560
        #base_learner其实只是一个（5， z_dim）的全连接层
        ...
        if self.mode == 'meta':
            ...
            self.pre_fc = nn.Sequential(nn.Linear(2560, 1000), nn.ReLU(), nn.Linear(1000, num_cls))

        self.ct = CenterLoss(self.args.way, 2560)  #center
    ...
    def meta_forward(self, data_shot, label_shot, data_query):
        ...
        # if center_weights == None:
        #     center_out, centers = self.ct(label_shot, embedding_shot, centers=center_weights)  #center
        #     center_weights = centers
        #     # print(center_weights)
        # else:
        center_out, center_weights = self.ct(label_shot, embedding_shot, centers=None)  # center
        logits = self.base_learner(embedding_shot)


        loss = F.cross_entropy(logits, label_shot) + miu * center_out #center

        center_grad = torch.autograd.grad(loss, self.ct.parameters())
        center_weights = list(map(lambda p: p[1] - alpha * p[0], zip(center_grad, center_weights)))   #center
        ...
        for _ in range(1, self.update_step):
            center_out, center_weights = self.ct(label_shot, embedding_shot, centers=center_weights)  # center

            logits = self.base_learner(embedding_shot, fast_weights)
            loss = F.cross_entropy(logits, label_shot) + miu * center_out   #center

            center_grad = torch.autograd.grad(loss, center_weights)
            center_weights = list(map(lambda p: p[1] - alpha * p[0], zip(center_grad, center_weights)))  # center
            ...
            logits_q = self.base_learner(embedding_query, fast_weights)
            # center_logits_q = self.ct(label_shot, embedding_query, centers=center_weights)

        return logits_q, embedding_query, center_weights
