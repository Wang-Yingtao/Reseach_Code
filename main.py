...
import argparse
import torch
from utils.misc import pprint #pprint打印出来的数据结构更为完整，每行为一个数据结构，对于比较长的数据会打印得较为清晰，而不是像print一样打印出一长串
from utils.gpu_tools import set_gpu #设定需要用到的GPU
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', type=int, default=999) # Manual seed for PyTorch, "0" means using random seed
    ...
    parser.add_argument('--test_way', type=int, default=5)  # Way number, how many classes in a task
    ...
    parser.add_argument('--meta_lr2', type=float, default=0.001) # Learning rate for FC weights   meta_lr1 meta_lr2其实就是γ，用于外循环参数的更新
    parser.add_argument('--base_lr', type=float, default=0.01) # Learning rate for the inner loop 预训练时参数更新的学习率 base_lr是β，用于内循环参数的更新
    ...
    pprint(vars(args)) #vars的作用相当于就是提取出对象中的字典,而pprint则是将字典按照结构化形式输出
    ...
    if args.seed==0:
        ...
        torch.backends.cudnn.benchmark = True  #这行代码的作用是找到一个卷积最好的方式，从而实现加速
    else:
