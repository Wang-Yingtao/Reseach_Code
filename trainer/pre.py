...
class PreTrainer(object):
    ...
    def __init__(self, args):
        ...
        args.save_path = pre_base_dir + '/' + save_path1 + '_' + save_path2  #args.save_path是parser直接新增的变量
        ensure_path(args.save_path)  #如果文件夹不存在，则新建

        # Set args to be shareable in the class
        self.args = args

        # Load pretrain set
        self.trainset = Dataset('pretrain', self.args, train_aug=True)    #利用dataset类把数据集制作成dataset格式，对图像进行变换，并取出数据及标记对应标签-getitem
        self.train_loader = DataLoader(dataset=self.trainset, batch_size=args.pre_batch_size, shuffle=True, num_workers=0, pin_memory=True) #将自定义的Dataset根据batch size大小、是否shuffle等封装成一个Batch Size大小的Tensor，用于后面的训练
        print('train_loader', len(self.train_loader)) #这里是300，意味着128*300张图片，一个 batch128个数据，总共300个


        # Load meta-val set
        self.valset = Dataset('test', self.args)
        #详解https://zhuanlan.zhihu.com/p/354214865
        #这里sampler的作用是用来采样出一个任务的规格的，N-way K-shot
        self.val_sampler = CategoriesSampler(self.valset.label, 200, self.args.way, self.args.shot + self.args.val_query)
        #根据任务的规格产生出600个batch的数据
        self.val_loader = DataLoader(dataset=self.valset, batch_sampler=self.val_sampler, num_workers=0, pin_memory=True)
        #print('self.val_loader',len(self.val_loader)) #这里是600，意味着产生了600个任务的数据，一个任务一个batch
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.pre_max_epoch + 1):
            ...
            self.model.train() #https://blog.csdn.net/weixin_43593330/article/details/107547202 启用BatchNormalization和 Dropout，将BatchNormalization和Dropout置为True
            self.model.mode = 'pre'
            ...
            for i, batch in enumerate(tqdm_gen, 1):
                ...
                if torch.cuda.is_available():
                    data, _ = [_.cuda() for _ in batch] #二者取前一，所以不用batch[0]
                else:
                    data = batch[0]
                label = batch[1]
                if torch.cuda.is_available():
            ...
            label = torch.arange(self.args.way).repeat(self.args.val_query) #[[],[],..,[]]复制val_query个[0,args.way] 得到(N-way, K-query)
            # print('label', label)
            if torch.cuda.is_available():
                label = label.type(torch.cuda.LongTensor)
            else:
